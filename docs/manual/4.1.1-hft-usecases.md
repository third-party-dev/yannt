...
The tool that can extract and rebuild untrained models from the huggingface transformers library is called `yannt_transformers` or `hft` as the CLI command.

## Installation

At the moment, there is no published wheel for this tool. It is recommended to `git clone` the repository and then within the top-level folder, run `./scripts/init-dev.sh`. This will create a `venv` python environment with the `yannt_transformers` package installed in place.
